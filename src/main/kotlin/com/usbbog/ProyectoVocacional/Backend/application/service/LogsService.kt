...
class LogsService (
    ...
    fun obtenerLogs(): List<Logs> {
        val logs = repository.obtenerTodos()

        if (logs.isEmpty()) throw ResponseStatusException(HttpStatus.NOT_FOUND,"No se encontraron Logs")
    ...
    fun generarLogLogin(
        ...
        val log = Logs(
            id = null,
            idUsuario = usuario?.id ?: 0, // revisar si tu BD permite 0/null
